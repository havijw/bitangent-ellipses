...
each coefficient of $Q_t$ is affine in $t$, it is just $\text{(coeffs of } L_0 L_1)
+ t \cdot(\text{coeffs of } M^2)$, stored as `P` and `S`.
...
therefore *is* the pencil. The single point the affine parameter misses ($t \to
\infty$) is the doubled chord $M^2 = 0$, the degenerate member where the ellipse
...
$$
B^2 - 4AC \;\begin{cases} < 0 & \text{ellipse (}\mathbf{M}_2\text{ definite)}\\ = 0 & \text{parabola}\\ > 0 & \text{hyperbola.}\end{cases}
$$
...
A short computation gives $F'$ without re-substituting. Since $\mathbf{M}_2 c =
-\tfrac12 \mathbf{b}$ (the center equation),
...
Let $V$ be the intersection of the two tangent lines and $\mathrm{mid} =
\tfrac12(P_0 + P_1)$ the chord midpoint. Define the *apex point*
...
($a \to 0$) to the parabola boundary ($a \to \tfrac12$). Beyond it, $a \in
(\tfrac12, 1)$ gives hyperbolas.
...
(`aFromU` / `uFromA`). Both endpoints $a \to 0$ and $a \to \tfrac12$ push $u \to
\mp\infty$, so a bounded scan in $u$ still probes arbitrarily close to the
...
apex parameter on $(0, \tfrac12)$, blowing up at both ends (a sliver near $a \to
0$, an infinitely elongated near-parabola near $a \to \tfrac12$), so it has an
...
Setting $B' = 0$ aligns the axes with the $\theta$ frame. Define $g(A, B, C) =
B\cos 2\theta - (A - C)\sin 2\theta$. Because $A, B, C$ are affine in $t$, so is
$g$:
...
rejected). Note $B'(\theta) = 0$ holds for the axes at $\theta$ *or* $\theta +
90°$, which is why the rotation solve is $90°$-periodic — the tool folds the
target into $[0°, 90°)$.

### 6.5 Aspect ratio — quadratic in $t$

Let the ratio be $k = r_{\text{major}}/r_{\text{minor}} \ge 1$. From §4.4, $r^2
\propto 1/\lambda$, so $k^2 = \lambda_{\max}/\lambda_{\min}$. Using the trace
...
because $\det \mathbf{M}_2(t)$ has no $t^2$ term — §5.2!), and writing $w =
(k^2+1)^2$, gives a genuine quadratic in $t$:
...
- **A tangent along the chord** ($d_0 \parallel P_0P_1$ or $d_1 \parallel
  P_0P_1$): then $P_0$ lies on $L_1$ (or $P_1$ on $L_0$), the factor $L_1(P_0)$ in
...
1. Build the three linear forms $L_0, L_1, M$ (§2) and the pencil $Q_t = L_0 L_1 +
   t M^2$ (§3).
